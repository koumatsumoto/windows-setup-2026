...
- `open` 関数が `.bashrc` に含まれている（WSL の場合）
...
# ---------------------
# open (WSL: Windows 側でファイル/フォルダを開く)
#   ディレクトリ      -> エクスプローラで開く
#   .html / .htm      -> 既定ブラウザで開く (HTML の関連付け経由)
...
if [ -n "$WSL_DISTRO_NAME" ]; then
  open() {
    ...
    if [ ! -e "$target" ]; then
      echo "open: not found -> $target" >&2
      return 1
    fi

    # local と代入を分けることで wslpath の失敗を検知する
    winpath="$(wslpath -w "$target")" || {
      echo "open: cannot convert path -> $target" >&2
      return 1
    }
...
WSL（Ubuntu）環境でのみ機能するように `$WSL_DISTRO_NAME` をチェックして定義している。Git Bash 等の Windows 側シェルでは、標準の `start` コマンドや `open` (macOS の場合) 相当の動作をするため定義しない。
